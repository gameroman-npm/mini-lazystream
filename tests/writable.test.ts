...
await test("writable - passes options and execution context", () => {
...
await test("writable - dummy underlying data population", () => {
...
await test("writable - streams2 on-demand write cycle", async () => {
  const expected = ["line1\n", "line2\n"];
  const actual: string[] = [];
