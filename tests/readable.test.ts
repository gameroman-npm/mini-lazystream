...
await test("readable - dummy data consistency", async () => {
...
await test("readable - passes options and execution context", () => {
...
await test("readable - streams2 on-demand read cycle", async () => {
  const expected = ["line1\n", "line2\n"];
  const actual: string[] = [];
...
await test("readable - resume flow handling", async () => {
  const expected = ["line1\n", "line2\n"];
  const actual: string[] = [];
  let instantiated = false;
